...
Time_int=0.5;  %Control Time Horizon
Ahead_H=8;     %Ratio: Predictaion/Control

c_jerk=70;    %35 %MPC-like parameters   
c_control=170;  %51
c_interd=2.5;    %4
c_jerk_a=35;
c_control_a=80;
...
alpha_s2=zeros(1,Time_simu/Time_int+1);
alpha_s3=zeros(1,Time_simu/Time_int+1);
alpha_s5=zeros(1,Time_simu/Time_int+1);
lvm01=zeros(1,Time_simu/Time_int);
lvm02=zeros(1,Time_simu/Time_int);
lvm03=zeros(1,Time_simu/Time_int);
con_break2=zeros(1,Time_simu/Time_int);
con_break3=zeros(1,Time_simu/Time_int);
con_break5=zeros(1,Time_simu/Time_int);
record_J2=zeros(1,Time_simu/Time_int);
record_J3=zeros(1,Time_simu/Time_int);
record_J5=zeros(1,Time_simu/Time_int);


time_orig=0:0.001:60; %inteplate the leading car's profile
for index_i=1:1:59/Time_int+1
    input1_a(index_i)=1.065*interp1(time_orig,input1,Time_int*(index_i-1)); %1.065
end
...
%% Simulation
for i=1:1:Time_simu/Time_int  
    ur2=input1_a(i:i+1)';%give the leading car's control input (for every time setp)
    ur0=input1_a(i:i+Ahead_H)';%give the leading car's control input (for every time horizon)
    j=0;
    
    %%%The MPC in 2nd car (CACC)
    for alpha=max(0,alpha_s2(i)-0.01):0.00125:min(alpha_s2(i)+0.015,1)  %Loop of alpha 0.01 /0.025
    ...
    [cos1,mv1,mv2,ur03,vm02] = mpc1_1(hdr2,ur0,initiala,Time_int,Ahead_H,c_jerk,c_control,c_interd);%%return the cost function value
...
j=0;
    %%The MPC in 3rd car
    for alpha=max(0,alpha_s3(i)-0.008):0.001:min(alpha_s3(i)+0.008,1) %0.003, 0.0012
        hdr3=alpha*hh3+(1-alpha)*ha;
        j=j+1;
        [cos2,mv3,mv4,ur04,vm03]=mpc2_1a(hdr3,ur3,initialb,Time_int,Ahead_H,c_jerk,c_control,c_interd);
    ...
    for alpha=max(0,alpha_s5(i)-0.006):0.002:min(alpha_s5(i)+0.005,1)
        ...
        j=j+1;
        [cos3,mv5,mv7,vm05]=mpc3_1b(hdr5,ur4,initialc,Time_int,Ahead_H,c_jerk_a,c_control_a,c_interd_a);
        coef51=1;
        coef52=1;
        pun51=0;
        pun52=0;
        ...
        vel7(i:i+Ahead_H)=mv7;
         ...
         if (1.03*no12a(z)>=no62(z))&&(1.328*no1fa(z)>=no6f(z))%&&(1.03*no12a(2)>=no62(2))&&(1.328*no1fa(2)>=no6f(2))&&(1.03*no12a(3)>=no62(3))&&(1.328*no1fa(3)>=no6f(3))&&(1.03*no12a(4)>=no62(4))&&(1.328*no1fa(4)>=no6f(4))&&(1.03*no12a(5)>=no62(5))&&(1.328*no1fa(5)>=no6f(5))%&&(1.03*no12a(6)>=no62(6))&&(1.328*no1fa(6)>=no6f(6))&&(1.03*no12a(7)>=no62(7))&&(1.328*no1fa(7)>=no6f(7))&&(1.03*no12a(8)>=no62(8))&&(1.328*no1fa(8)>=no6f(8))&&(1.03*no12a(9)>=no62(9))&&(1.328*no1fa(9)>=no6f(9))
             coef52_a=1;
             pun52_a=0;
         else 
             coef52_a=1e10;
             pun52_a=p_ratio*max(no62(z)/(1.03*no12a(z)),1)*max(no6f(z)/(1.328*no1fa(z)),1);
         end
    ...
    hd3=hd_s3;
    hd5=hd_s5;
    [H1,initia11]=overall(hd2,hd3,hd5,ur2,initial0,Time_int);%%six car real time simulation
    initial0=initia11;
    initiala=initial0(1:8);
    initialb=initial0(5:16);
    initialc=initial0(9:28);
    
    res(i:i+1,:)=H1; %record the data (velocity, jerk...)
    vel1=res(:,15);
    vel2=res(:,16);
    vel3=res(:,17);
    vel4=res(:,18);
 ...
 figure (1)                %get plot
subplot(4,1,1)
plot(Time_g,res(:,2),Time_g,res(:,3),Time_g,res(:,5))
legend('car2','car3','car5')
ylabel('jerk(m/s^3)')
title('jerk')
%ylim([-0.5,0.5])
subplot(4,1,2)
plot(Time_g,res(:,9),Time_g,res(:,10),Time_g,res(:,12))
legend('car2','car3','car5')
ylabel('control input(m/s^2)')
title('control input')
subplot(4,1,3)
plot(Time_g,res(:,15),Time_g,res(:,16),Time_g,res(:,17),Time_g,res(:,18),Time_g,res(:,19),Time_g,res(:,20),Time_g,res(:,21))
legend('car1','car2','car3','car4','car5','car6')
title('velocity(m/s)')
ylabel('velocity(m/s)')
subplot(4,1,4)
stairs(Time_g(1:length(Time_g)-1),con_break2)
hold on
stairs(Time_g(1:length(Time_g)-1),con_break3)
hold on
stairs(Time_g(1:length(Time_g)-1),con_break5)
...
norm_control_2=norm(res(:,10));
norm_control_1=norm(res(:,9));
norm_control_3=norm(res(:,12));

norm_jerk_1=norm(res(:,2));
norm_jerk_2=norm(res(:,3));
norm_jerk_3=norm(res(:,5));

norm_interd_1=norm(res(:,22));
norm_interd_2=norm(res(:,23));
norm_interd_3=norm(res(:,25));
